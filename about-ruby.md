...
出来ます。
また、<span style="color: red; ">1 行目の ``# coding: cp932`` という文字は
...
上記プログラムの通り、"配列" は ``[]`` によって定義されます。<span style="color: red; ">配列を使うときは、必ず定義しましょう。</span>また、
配列名の末尾に ``[数値]`` をつけることで、
...
<span style="color: green; ">ヒント:</span>
```ruby
array[0]=1
array[1]=2
array[2]=3
...
<span style="color: green; ">さらにヒント：</span>
```ruby
count = 0
loop do
　array[count] = count + 1
